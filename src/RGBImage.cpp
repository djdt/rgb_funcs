...
namespace rgb
{
	...
	bool Image::loadPngFile(const std::string& file)
	{
		png_byte png_sig[8];
		...
		if (!ifs.good()) {
			std::cerr << "Image: source is not good!" << std::endl;
			return false;
		}
		...
		if (png_sig_cmp(png_sig, 0, 8)) {
			std::cerr << "Image: png signature invalid!" << std::endl;
			return false;
		}
		...
		if (!png_ptr) {
			std::cerr << "Image: failed to create png struct!" << std::endl;
			return false;
		}
		...
		if (!info_ptr) {
			...
			std::cerr << "Image: failed to create png info!" << std::endl;
			return false;
		}
		...
		for (uint32_t i = 0; i < _height * _width; ++i) {
			/* _pixels.push_back( */
			/* 		pixel(img_data[(i*3)], img_data[(i*3)+1], img_data[(i*3)+2])); */
	...
	bool Image::loadJpegFile(const std::string& file)
	{
